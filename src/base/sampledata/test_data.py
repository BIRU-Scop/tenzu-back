...
from base.sampledata import constants, factories
...
async def load_test_data() -> None:
    ...
    custom_user = users[0]
    workspace = await factories.create_workspace(created_by=custom_user, name="Custom workspace")
    await _create_empty_project(created_by=custom_user, workspace=workspace)
    await _create_inconsistent_permissions_project(created_by=custom_user, workspace=workspace)
    await _create_project_with_several_roles(created_by=custom_user, workspace=workspace, users=users)
    ...
    await _create_scenario_with_1k_stories(workspace=workspace, created_by=custom_user, users=all_users)
...
async def _create_inconsistent_permissions_project(created_by: User, workspace: Workspace) -> None:
...
async def _create_project_with_several_roles(created_by: User, workspace: Workspace, users: list[User]) -> None:
    project = await factories.create_project(name="Several Roles", created_by=created_by, workspace=workspace)
...
async def _create_project_membership_scenario() -> None:
    ...
    await ws_memberships_repositories.create_workspace_membership(user=user1001, workspace=workspace)
    ...
    workspace = await factories.create_workspace(created_by=user1000, name="uk/uk1 (ws-member), uk2/uk3 (ws-guest)")
    await ws_memberships_repositories.create_workspace_membership(user=user1001, workspace=workspace)
    ...
    workspace = await factories.create_workspace(created_by=user1000, name="uk-ws-member uk1/k2-ws-mb uk3-ws-guest")
    await ws_memberships_repositories.create_workspace_membership(user=user1001, workspace=workspace)
    await ws_memberships_repositories.create_workspace_membership(user=user1002, workspace=workspace)
    # project: u1k pj-admin, u1k2 pj-member view_story, u1k1/u1k3 no pj-member, public view-us, ws-members not-allowed
    project = await factories.create_project(
        workspace=workspace,
        created_by=user1000,
...
async def _create_scenario_with_invitations() -> None:
    ...
    ws1 = await workspaces_services._create_workspace(name="ws1 for members", created_by=user900, color=2)
    ...
    await ws_memberships_repositories.create_workspace_membership(user=user901, workspace=ws1)

    # user901 is member of ws2
    await ws_memberships_repositories.create_workspace_membership(user=user901, workspace=ws2)

    # user901 is member of ws3
    await ws_memberships_repositories.create_workspace_membership(user=user901, workspace=ws3)
...
async def _create_scenario_for_searches() -> None:
    ...
    await ws_memberships_repositories.create_workspace_membership(user=elettescar, workspace=ws1)
...
async def _create_scenario_for_revoke() -> None:
    ...
    await ws_memberships_repositories.create_workspace_membership(user=user4, workspace=ws)
    await ws_memberships_repositories.create_workspace_membership(user=user2, workspace=ws)
...
async def _create_scenario_with_1k_stories(workspace: Workspace, users: list[User], created_by: User) -> None:
    """
