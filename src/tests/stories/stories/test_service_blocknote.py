...
async def test_convert_blocks_data():
    ...
    async with converter:
        string_id, binary_data, block_data = await converter.convert(
            {"id": "0", "content": data}
        )
    assert converter._process.returncode is not None
    assert string_id == "0"
    assert isinstance(binary_data, bytes)
    assert block_data is None


async def test_convert_html_data():
...
async def test_convert_markdown_data():
    data = "*text* \nIn **markdown**\n#Title\n- and\n- a\n- list\n"
