...
from projects.memberships.api import UpdateRoleValidator
...
def test_validate_role_ok():
    ...
    patch = UpdateRoleValidator(name=name, permissions=permissions)
...
def test_validate_role_ko_empty_permission():
    ...
    with pytest.raises(ValidationError) as validations_errors:
        UpdateRoleValidator(name=name, permissions=permissions)
